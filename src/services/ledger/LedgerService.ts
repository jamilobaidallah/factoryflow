  ...
  async updateLedgerEntry(
    ...
    try {
      ...
      if (existingTransactionId) {
        ...
        // Strategy: use the inventory movement as the PRIMARY trigger for COGS re-creation.
        // The COGS ledger entry query is SECONDARY — only used to delete the old entry and
        // retrieve the original amount as a fallback. This prevents the bug where the COGS
        // journal is reversed but not re-posted when the COGS ledger entry query returns empty.
        ...
        const [movementSnapshot, cogsLedgerSnapshot] = await Promise.all([
          getDocs(movementQuery),
          getDocs(cogsLedgerQuery),
        ]);

        // Delete old COGS ledger entry if found
        let originalCogsAmount = 0;
        if (!cogsLedgerSnapshot.empty) {
          originalCogsAmount = cogsLedgerSnapshot.docs[0].data()?.amount || 0;
          cogsLedgerSnapshot.forEach((cogsDoc) => {
            batch.delete(cogsDoc.ref);
          });
        }

        // Re-create COGS whenever there is an outgoing inventory movement (sale).
        // This does NOT depend on the COGS ledger entry existing — fixing the regression
        // where editing a sale caused inventory to revert on the balance sheet.
        if (!movementSnapshot.empty && movementSnapshot.docs[0].data().type === "خروج") {
          const movementData = movementSnapshot.docs[0].data();

          // Get current inventory cost for COGS recalculation
          const inventoryItemRef = doc(
            firestore,
            `users/${this.userId}/inventory`,
            movementData.itemId
          );
          const inventorySnap = await getDoc(inventoryItemRef);
          ...
          if (currentUnitCost > 0) {
            newCogsAmount = safeMultiply(movementData.quantity, currentUnitCost);
            cogsNotes = `حساب تلقائي: ${movementData.quantity} × ${roundCurrency(currentUnitCost).toFixed(2)} = ${roundCurrency(newCogsAmount).toFixed(2)} دينار`;
          } else if (originalCogsAmount > 0) {
            // Inventory item deleted or has zero price — preserve the stored COGS amount
            newCogsAmount = originalCogsAmount;
          ...
          if (newCogsAmount > 0) {
            const cogsDescription = `تكلفة البضاعة المباعة - ${movementData.itemName}`;
        ...
        if (currentData) {
          ...
          await this.postJournalEntry(engine, {
            ...
            context: {
              ...
              // Preserve inventory purchase flag so purchase edits re-journal to 1300 (asset)
              // instead of the expense account (e.g., 5010). Without this, editing a purchase
              // entry would switch the journal from DR Inventory to DR Expense.
              isInventoryPurchase: currentData.isInventoryPurchase ?? false,
            },
