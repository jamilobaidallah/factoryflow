  ...
  async updateLedgerEntry(
    ...
    try {
      ...
      if (existingTransactionId) {
        ...
        // Skip for sales returns — the 4-line compound journal already handles the COGS leg,
        // and the isCOGSReversal doc must NOT be deleted (it's the P&L record for the return).
        const isSalesReturnEdit = formData.category === "مردودات المبيعات";
        const cogsLedgerQuery = isSalesReturnEdit ? null : query(
          this.ledgerRef,
          where("linkedTransactionId", "==", existingTransactionId),
          where("autoGenerated", "==", true)
        );
        const cogsLedgerSnapshot = cogsLedgerQuery ? await getDocs(cogsLedgerQuery) : null;
        ...
        if (cogsLedgerSnapshot && !cogsLedgerSnapshot.empty) {
