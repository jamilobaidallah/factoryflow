      ...
      if (existingTransactionId) {
        ...
        if (cogsRecreationData) {
          ...
          if (currentUnitCost > 0) {
            const newCogsAmount = safeMultiply(cogsRecreationData.quantity, currentUnitCost);
            ...
            batch.set(cogsDocRef, {
              ...
              linkedTransactionId: existingTransactionId,
              autoGenerated: true,
              notes: `حساب تلقائي: ${cogsRecreationData.quantity} × ${roundCurrency(currentUnitCost).toFixed(2)} = ${roundCurrency(newCogsAmount).toFixed(2)} دينار`,
              createdAt: new Date(),
            });
