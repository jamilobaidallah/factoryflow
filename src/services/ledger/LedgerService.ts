      ...
      if (existingTransactionId) {
        ...
        if (!cogsLedgerSnapshot.empty) {
          ...
          if (!movementSnapshot.empty) {
            ...
            // Fallback: inventory movement not found, but COGS entry exists with a valid amount.
            // Re-create COGS directly using the stored amount to prevent the COGS journal reversal
            // (which happens after batch commit) from leaving account 1300 unbalanced.
            // Without this, the COGS reversal creates an uncompensated DR 1300, causing the
            // balance sheet inventory to revert to its pre-sale value.
            const fallbackDescription = originalCogsData?.description || 'تكلفة البضاعة المباعة';
            const fallbackCogsDocRef = doc(this.ledgerRef);
            batch.set(fallbackCogsDocRef, {
              transactionId: `COGS-${existingTransactionId}`,
              description: fallbackDescription,
              type: "مصروف",
              amount: originalCogsAmount,
              category: "تكلفة البضاعة المباعة (COGS)",
              subCategory: "مبيعات",
              date: new Date(formData.date),
              linkedTransactionId: existingTransactionId,
              autoGenerated: true,
              notes: originalCogsData?.notes || '',
              createdAt: new Date(),
            });
            // Set cogsRecreationResult directly — the cogsRecreationData block below is skipped
            cogsRecreationResult = { description: fallbackDescription, amount: originalCogsAmount };
          }
        }
