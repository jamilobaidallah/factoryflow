...
export class LedgerService {
  ...
  /**
   * Creates a COGS (Cost of Goods Sold) ledger entry in the batch
   */
  private addCOGSRecord(
    batch: WriteBatch,
    transactionId: string,
    itemName: string,
    quantity: number,
  ...
  /**
   * Creates a new inventory item in the batch
   */
  private createNewInventoryItemInBatch(
    batch: WriteBatch,
    inventoryFormData: InventoryFormData,
    formData: LedgerFormData,
  ...
  /**
   * Adds an inventory movement record to the batch
   */
  private addMovementRecordToBatch(
    batch: WriteBatch,
    itemId: string,
    inventoryFormData: InventoryFormData,
    movementType: string,
  ...
  private async handleInventoryUpdateBatch(
    ...
    if (!itemSnapshot.empty) {
      ...
      try {
        ...
        // Handle transaction errors with user-friendly messages using typed errors
      ...
      itemId = this.createNewInventoryItemInBatch(batch, inventoryFormData, formData, quantityChange);
    }

    // Add movement record (write-only, safe to use batch)
    this.addMovementRecordToBatch(
      batch,
      itemId,
      inventoryFormData,
      movementType,
