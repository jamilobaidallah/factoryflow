...
import { Clock, CheckCircle, RotateCcw, ArrowLeftRight, Ban } from "lucide-react";
import { formatNumber } from "@/lib/date-utils";

interface OutgoingChequesSummaryHeaderProps {
  stats: {
    pendingCount: number;
    pendingValue: number;
    clearedCount: number;
...
function OutgoingChequesSummaryHeaderComponent({
  stats,
  loading,
  actions,
}: OutgoingChequesSummaryHeaderProps) {
  return (
      ...
      <div className="flex flex-col gap-4 p-4 bg-gradient-to-l from-slate-50/90 to-white rounded-xl border border-slate-200/60 shadow-sm">
        ...
        <div className="flex flex-col xl:flex-row xl:items-center gap-4">
          ...
          <div className="flex-shrink-0">
            <h1 className="text-2xl font-bold text-slate-800">الشيكات الصادرة</h1>
            <p className="text-sm text-slate-500">إدارة الشيكات الصادرة للموردين</p>
          </div>
          ...
          <div className="flex items-center gap-3 flex-1 flex-wrap xl:flex-nowrap">
            {loading ? (
              <>
                ...
                <div className="h-14 flex-1 min-w-[100px] bg-slate-200 rounded-lg animate-pulse" />
                <div className="h-14 flex-1 min-w-[80px] bg-slate-200 rounded-lg animate-pulse" />
                <div className="h-14 flex-1 min-w-[80px] bg-slate-200 rounded-lg animate-pulse" />
                <div className="h-14 flex-1 min-w-[80px] bg-slate-200 rounded-lg animate-pulse" />
              </>
            ) : (
              <>
                {/* Pending */}
                <div className="flex items-center gap-3 px-4 py-2.5 bg-amber-50/80 rounded-lg border border-amber-100 flex-1 min-w-[110px]">
                ...
                {/* Returned */}
                <div className="flex items-center gap-3 px-4 py-2.5 bg-rose-50/80 rounded-lg border border-rose-100 flex-1 min-w-[90px]">
                  <div className="flex items-center justify-center w-10 h-10 bg-rose-100 rounded-lg flex-shrink-0">
                    <RotateCcw className="w-5 h-5 text-rose-600" strokeWidth={2} />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-xs text-rose-600/80 font-medium">مرتجع</span>
                ...
                {/* Endorsed */}
                <div className="flex items-center gap-3 px-4 py-2.5 bg-purple-50/80 rounded-lg border border-purple-100 flex-1 min-w-[90px]">
                  ...
                  <div className="flex flex-col">
                    <span className="text-xs text-purple-600/80 font-medium">مظهرة</span>
                ...
                {/* Cancelled */}
                <div className="flex items-center gap-3 px-4 py-2.5 bg-slate-50/80 rounded-lg border border-slate-200 flex-1 min-w-[90px]">
                  <div className="flex items-center justify-center w-10 h-10 bg-slate-100 rounded-lg flex-shrink-0">
                    <Ban className="w-5 h-5 text-slate-500" strokeWidth={2} />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-xs text-slate-500/80 font-medium">ملغي</span>
                    <span className="text-lg font-bold text-slate-600 tabular-nums leading-tight">
...
export const OutgoingChequesSummaryHeader = memo(OutgoingChequesSummaryHeaderComponent);
